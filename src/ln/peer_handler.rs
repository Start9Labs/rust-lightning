...
impl<Descriptor: SocketDescriptor> PeerManager<Descriptor> {
	...
	fn get_ephemeral_key(&self) -> SecretKey {
		...
		SecretKey::from_slice(&Sha256::from_engine(ephemeral_hash).into_inner())
	...
	fn do_attempt_write_data(&self, descriptor: &mut Descriptor, peer: &mut Peer<Complete>) {
		...
		macro_rules! encode_and_send_msg {
			($msg: expr, $msg_code: expr) => {{
				log_trace!(
					self,
					"Encoding and sending sync update message of type {} to {}",
					$msg_code,
			...
			if peer.pending_outbound_buffer.len() < MSG_BUFF_SIZE {
				match peer.sync_status {
					...
					InitSyncTracker::ChannelsSyncing(c) if c < 0xffff_ffff_ffff_ffff => {
						let steps =
							((MSG_BUFF_SIZE - peer.pending_outbound_buffer.len() + 2) / 3) as u8;
...
mod tests {
	...
	fn create_network(peer_count: usize) -> Vec<PeerManager<FileDescriptor>> {
		...
		for _ in 0..peer_count {
			...
			let peer =
				PeerManager::new(msg_handler, node_id, &ephemeral_bytes, Arc::clone(&logger));
			peers.push(peer);
		}
	...
	fn test_disconnect_peer() {
		...
		chan_handler
			...
			.lock()
			.unwrap()
			.push(events::MessageSendEvent::HandleError {
				node_id: their_id,
